...
public class UserController {
    ...
    @GetMapping("/")
    public UserResponseDTO getUserById(@RequestHeader("X-User-Id") String passedUserId) {
    ...
    @GetMapping("/nickname")
    public String getUserNicknameById(@RequestHeader("X-User-Id") String passedUserId) {
        Long userId = Long.valueOf(passedUserId);
    ...
    @GetMapping("/turtle")
    public ResponseEntity<?> getTurtlesByUserId(@RequestHeader("X-User-Id") String passedUserId) {
        Long userId = Long.valueOf(passedUserId);
        return userService.getTurtlesByUserId(userId);
    }

    // !~!~userId 토큰에서 가져오는거 여기 참고하기~!~!
    @GetMapping("/transaction")
    public ResponseEntity<?> myTransaction(@RequestHeader("X-User-Id") String passedUserId){
        Long userId = Long.valueOf(passedUserId);
        User user = userRepository.findById(userId).orElseThrow(() -> new UserNotFoundException("사용자를 찾을 수 없습니다.")); // token 기준으로 User 객체 가져오기!
        return new ResponseEntity<>(ResponseVO.success("요청이 정상적으로 처리되었습니다.","transaction",transactionService.findAllTransactions(user)),HttpStatus.OK);
    }
    ...
    public ResponseEntity<?> modifyMyProfileImage(
        ...
        try{
            ...
            if(user.getProfileImage() != null){
                try{
                    imageUploadService.deleteS3(user.getProfileImage());
                }catch (Exception e){
